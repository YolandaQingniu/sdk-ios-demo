...
typedef NS_ENUM(NSUInteger, QNPScaleState) {
    ...
    QNPScaleStateWriteModelBegin,         //开始写入型号
    QNPScaleStateWriteModelSuccess,       //写入型号成功
    QNPScaleStateWriteModelFail,          //写入型号失败
    QNPScaleStateConfirmDeviceMode,       //确认秤的类型
    ...
    QNPScaleStateHardwareParamsError,     //硬件参数有异常(可以忽视)
    QNPScaleStateEnterOTAMode,            //设备进入OTA模式
};
...
typedef NS_ENUM(NSUInteger, QNScaleDeviceCode) {
    ...
    QNScaleDeviceUnsupportSetupNetworkError, //不支持配网
    QNScaleDeviceHardwareParamsError, //硬件参数出错
};

typedef NS_ENUM(NSUInteger, QNPScaleOTAState) {
    QNPScaleOTAStateDefault = 0,     //默认
    QNPScaleOTAStateUpdating,        //升级中
    QNPScaleOTAStateUpdateComplete,  //升级完成
    QNPScaleOTAStateUpdateFailed,    //升级失败
};

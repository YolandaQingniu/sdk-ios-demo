...
+ (void)AES128Encrypt:(NSString *)plainText result:(void (^)(NSString *))block {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSString *result = [self AES128Encrypt:plainText];
...
+ (void)AES128Decrypt:(NSString *)encryptText result:(void (^)(NSString *))block {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSString *result = [self AES128Decrypt:encryptText];
        dispatch_async(dispatch_get_main_queue(), ^{
            if (block) {
                block(result);
            }
...
+ (void)encrypt:(NSString *)message password:(NSString *)password result:(void (^)(NSString *))block {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSString *result = [self encrypt:message password:password];
        dispatch_async(dispatch_get_main_queue(), ^{
            if (block) {
                block(result);
            }
...
+ (void)decrypt:(NSString *)base64EncodedString password:(NSString *)password result:(void (^)(NSString *))block {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSString *result = [self decrypt:base64EncodedString password:password];
        dispatch_async(dispatch_get_main_queue(), ^{
            if (block) {
                block(result);
            }

...
#define BandMMKVUserIdKey @"userIdKey"
...
- (QNUser *)user {
    ...
    if (data != nil) {
        ...
        user.userId = [dis valueForKey:BandMMKVUserIdKey];
        ...
        user.birthday = [[NSCalendar currentCalendar] dateFromComponents:dateComponents];
    }
    return user;
}
...
- (void)setUser:(QNUser *)user {
    ...
    [userDis setObject:user.userId forKey:BandMMKVUserIdKey];
...
- (void)setSitRemind:(QNSitRemind *)sitRemind {
    ...
    [sitRemindDis setObject:[NSNumber numberWithInt:sitRemind.timeInterval] forKey:BandMMKVSitRemindTimeIntervalKey];
    int repeatWeek = 0;
